...
| URI | Description |
| --- | --- |
| `config://app` | Static JSON server config & feature flags. |
...
| `GET` | `/mcp` | Server-Sent Events stream for server→client notifications. |
| `DELETE` | `/mcp` | Terminate a session. |
| `GET` | `/health` | Plain health check (not part of MCP). |

Sessions are tracked via the `Mcp-Session-Id` response/request header. The port
defaults to **3000** and can be overridden:
...
The response includes an `Mcp-Session-Id` header — pass it back as a request
...
To add your own capability, edit the matching `register*` function in
`tools.ts`, `resources.ts`, or `prompts.ts` — they each receive the `McpServer`
instance and call `server.registerTool` / `registerResource` / `registerPrompt`.

---

## Notes

- **No authentication.** This server is intended for local testing and learning
  only — do not expose it to untrusted networks.
- When using stdio, **never** write to `stdout` — it's reserved for the JSON-RPC
  protocol. Diagnostics go to `stderr` (`console.error`).
